...
SELECT staff.first_name, staff.last_name, sum(payment.amount) as "Sum Charged"
FROM staff
INNER JOIN payment
...
address.address_id, city.city_id, country.country_id,country.country_id
FROM customer
INNER JOIN address
USING (address_id)
INNER JOIN city
...
SELECT staff.store_id, sum(payment.amount) AS "Total Revenue Per Store"
FROM staff
...
SELECT category.name, sum(payment.amount) AS "Revenue by Genre"
FROM category
...
SELECT * FROM sakila.top_by_genre;
